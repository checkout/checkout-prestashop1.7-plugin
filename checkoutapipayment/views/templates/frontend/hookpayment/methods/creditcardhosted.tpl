...
<form id="cko-payment-form" action="{$hppUrl}" method="POST">
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            jQuery('input:radio[name="payment-option"]').change(function(){
                if($(this).attr('data-module-name') == 'creditcardhosted'){
                    jQuery('button.btn.btn-primary.center-block').click(function(event){
                        if(jQuery("input:radio[name='payment-option']:checked").attr('data-module-name') == "creditcardhosted"){
                            ...
                            if(jQuery('.checkoutapipayment-saved-card').length > 0 &&
                            	...
                            	if(jQuery('#save-card-checkbox').is(':checked')){
				                    document.cookie = "saveCardCheckbox=1";
				                }
                                document.getElementById('cko-payment-form').submit();    
                            } 
                        }
                    });
                } else {
